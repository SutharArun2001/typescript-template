...
import iaSearchMainCss from "../src/style/css/ia-search-main-css.js";
import { 
  date, 
  mediaTypeOption, 
  searchFieldOption,
  ...
  years,
  months
 } from "../src/core/config/dropdown-fields";
...
export class IaSearchMain extends LitElement {
  ...
  formSubmit() {
    ...
    let querydata = encodeURIComponent(this.searchQuery);
    console.log(this.searchQuery)
    console.log(querydata)
    window.location.href = `search?query=${querydata}`;
        ...
        if (select_field === "none" || select_value === "" || select_value === "none") {
          ...
          if (select_field === 'date') {
            ...
            if ( [year, month, date].includes('none')) {
              ...
              var searchQToShow: string = `date:${year}-${month}-${date}`;
            }
            ...
            if ([yearFrom, monthFrom, dateFrom,yearTo,monthTo, dateTo].includes('none')) {
              ...
              var searchQToShow: string = `date:[${yearFrom}-${monthFrom}-${dateFrom} TO ${yearTo}-${monthTo}-${dateTo}]`;
            }
            
          } else {
            const select_condition = (element.querySelector('.select-condition') as HTMLInputElement).value;
            var searchQToShow: string = `${(select_condition === 'false') ? '-' + select_field : select_field}:(${select_value})`;
          }
